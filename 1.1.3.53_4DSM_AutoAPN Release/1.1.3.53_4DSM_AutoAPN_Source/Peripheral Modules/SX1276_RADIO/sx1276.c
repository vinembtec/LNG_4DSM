...
void SX1276SetLoRaOn( bool enable )
{
    ...
    if( LoRaOn == true )
    {
    	...
    	/*GPIO_setOutputLowOnPin(SPI_IOPORT,NSS_PIN);
    			//for(i=0;i<1000; i++);
		EUSCI_B_CMSIS(EUSCI_B2_BASE)->TXBUF = 0x81;
		 while( EUSCI_B_SPI_isBusy(EUSCI_B2_BASE));
		//for(i=0;i<100; i++);
		EUSCI_B_CMSIS(EUSCI_B2_BASE)->TXBUF = 0x83;
		 while( EUSCI_B_SPI_isBusy(EUSCI_B2_BASE));
		byte = EUSCI_B_CMSIS(EUSCI_B2_BASE)->RXBUF;
		GPIO_setOutputHighOnPin(SPI_IOPORT,NSS_PIN);

        GPIO_setOutputLowOnPin(SPI_IOPORT,NSS_PIN);
		//for(i=0;i<1000; i++);
		EUSCI_B_CMSIS(EUSCI_B2_BASE)->TXBUF = 0x01;
		 while( EUSCI_B_SPI_isBusy(EUSCI_B2_BASE));
		//for(i=0;i<100; i++);
		EUSCI_B_CMSIS(EUSCI_B2_BASE)->TXBUF = 0x00;
		 while( EUSCI_B_SPI_isBusy(EUSCI_B2_BASE));
		byte = EUSCI_B_CMSIS(EUSCI_B2_BASE)->RXBUF;
		GPIO_setOutputHighOnPin(SPI_IOPORT,NSS_PIN);*/
